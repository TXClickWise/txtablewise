-- Add is_live flag to restaurants
ALTER TABLE public.restaurants
  ADD COLUMN IF NOT EXISTS is_live boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS marked_live_at timestamptz;

-- Owner-only purge of operational data for a restaurant
...
BEGIN
  ...
  -- Delete child tables first (FK-safe ordering)
  ...
  DELETE FROM public.reservation_reminders WHERE restaurant_id = _restaurant_id;
  ...
  -- waitlist_entries may or may not exist depending on migrations
  BEGIN
...
REVOKE ALL ON FUNCTION public.purge_restaurant_operational_data(uuid) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.purge_restaurant_operational_data(uuid) TO authenticated;